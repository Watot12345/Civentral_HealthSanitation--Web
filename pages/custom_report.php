...
<main class="flex-1 bg-dash-bg h-screen m-5 rounded-2xl font-sans overflow-y-auto scrollbar-track-transparent">
    ...
    <!-- ─── RECENT REPORTS CARD (REMOVED ACTIONS COLUMN) ─── -->
...
function renderRecentReports() {
    ...
    tbody.innerHTML = list.map(r => `
        <tr class="table-row-hover">
            ...
            <td class="py-2.5"><span class="status-badge ${recentStatusBadge(r.status)} px-2 py-1 rounded-full text-xs">${r.status}</span></td>
        </tr>
    `).join('');
}
